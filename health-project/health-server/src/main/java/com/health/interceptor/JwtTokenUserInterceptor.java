...
public class JwtTokenUserInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        //如果是注册、登录接口，直接放行
        if ("/user/login".equals(request.getRequestURI()) || "/user/register".equals(request.getRequestURI()) || "/user/predict/DM".equals(request.getRequestURI()) || "/user/certify".equals(request.getRequestURI()))  {
            return true;
        }
        //1、从请求头中获取令牌
